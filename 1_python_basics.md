...
今まではPythonインタープリタの対話モード上でPythonのコードを直接実行していましたが、少し長い処理を書くには不便です。Python インタープリタの対話モードを終了し（macOS/Linuxの場合は【Ctrl】+【D】、Windowsの場合は【Ctrl】+【Z】+ Enter を入力）、エディタを起動してtest.pyというファイルを作成します。 
